...
public class TopicManagementTest extends IntegrationTest {
    ...
    public void shouldCreateTopic() {
        ...
        Response response = management.topic().create(
                topic("createTopicGroup", "topic").buildAsTopicWithSchema());
    ...
    public void shouldRecreateTopicAfterDeletion() {
        ...
        // when
        Response response = management.topic().create(
                topic("recreateTopicGroup", "topic").buildAsTopicWithSchema());
    ...
    public void shouldNotCreateInvalidTopic() {
        ...
        // when
        Response response = management.topic().create(topic("invalidTopicGroup", "topic").withRetentionTime(-1).buildAsTopicWithSchema());
    ...
    public void shouldNotAllowOnCreatingSameTopicTwice() {
        ...
        // when
        Response response = management.topic().create(topic("overrideTopicGroup", "topic").buildAsTopicWithSchema());
    ...
    public void shouldReturnTrackedTopicsWithAvroContentType() {
        // given
        operations.buildTopicWithSchema(topic("avroGroup", "avroTopic").withContentType(AVRO).withTrackingEnabled(false).buildAsTopicWithSchema(SCHEMA));
        operations.buildTopic(topic("jsonGroup", "jsonTopic").withContentType(JSON).withTrackingEnabled(false).build());
        operations.buildTopicWithSchema(topic("avroGroup", "avroTrackedTopic").withContentType(AVRO).withTrackingEnabled(true).buildAsTopicWithSchema(SCHEMA));
    ...
    public void shouldReturnTrackedTopicsWithAvroContentTypeForGivenGroup() {
        // given
        operations.buildTopicWithSchema(topic("mixedTrackedGroup", "avroTopic").withContentType(AVRO).withTrackingEnabled(false).buildAsTopicWithSchema(SCHEMA));
        operations.buildTopic(topic("mixedTrackedGroup", "jsonTopic").withContentType(JSON).withTrackingEnabled(false).build());
        operations.buildTopicWithSchema(topic("mixedTrackedGroup", "avroTrackedTopic").withContentType(AVRO).withTrackingEnabled(true).buildAsTopicWithSchema(SCHEMA));
    ...
    public void shouldNotAllowDollarSign() {
        ...
        Stream.of("$name", "na$me", "name$").forEach(topic -> {
            // when
            Response response = management.topic().create(
                    topic("dollar", topic).buildAsTopicWithSchema());
    ...
    public void shouldCreateTopicEvenIfExistsInBrokers() {
        ...
        // when
        Response response = management.topic().create(
                topic(groupName, topicName).buildAsTopicWithSchema());
    ...
    public void topicCreationRollbackShouldNotDeleteTopicOnBroker() throws Throwable {
        ...
        // when
        management.topic().create(topic(groupName, topicName).buildAsTopicWithSchema());
    ...
    public void shouldCreateTopicWithMaxMessageSize() {
        // given
        TopicWithSchema topic = TopicBuilder.topic("messageSize", "topic").withMaxMessageSize(2048).buildAsTopicWithSchema();
    ...
    public void shouldCreateTopicWithRestrictedSubscribing() {
        // given
        TopicWithSchema topic = TopicBuilder.topic("restricted", "topic").withSubscribingRestricted().buildAsTopicWithSchema();
        assertThat(management.group().create(new Group(topic.getName().getGroupName(), "a"))).hasStatus(CREATED);
